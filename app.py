...
GROQ_API_KEY = "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"
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .header-info h2 { font-size: 18px; font-weight: 600; color: #fff; }
        ...
        .mute-btn { background: rgba(255,255,255,0.1); border: none; color: white; width: 35px; height: 35px; border-radius: 50%; cursor: pointer; display: flex; justify-content: center; align-items: center; }
        ...
        .bubble { max-width: 80%; padding: 14px 18px; border-radius: 18px; font-size: 15px; line-height: 1.6; word-wrap: break-word; }
        ...
        .input-wrapper { flex: 1; background: #334155; border-radius: 14px; border: 1px solid rgba(255,255,255,0.1); display: flex; align-items: center; padding: 0 15px; }
        ...
        .action-btn { background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; border: none; width: 48px; height: 48px; border-radius: 14px; cursor: pointer; display: flex; justify-content: center; align-items: center; font-size: 18px; }
        #micBtn { background: linear-gradient(135deg, #ec4899 0%, #d946ef 100%); }
...
<body>
    <div id="chat-container">
        <div class="chat-header">
            ...
            <div class="voice-controls">
                <button class="mute-btn" id="audioToggle" onclick="toggleAudio()"><i class="fa-solid fa-volume-high"></i></button>
            </div>
        </div>
        <div id="messages-body">
            <div class="welcome-screen" id="welcome">
                ...
                <p>Pink Mic icon dabao ya type karein.</p>
            </div>
        </div>
        <div class="chat-input-area">
            <button class="action-btn" id="micBtn" onclick="startSpeechRecognition()"><i class="fa-solid fa-microphone"></i></button>
    ...
    <script>
        ...
        function toggleAudio() {
            ...
            if(isVoiceEnabled) { btn.innerHTML = '<i class="fa-solid fa-volume-high"></i>'; btn.classList.remove("active"); }
            else { btn.innerHTML = '<i class="fa-solid fa-volume-xmark"></i>'; btn.classList.add("active"); if(window.speechSynthesis) window.speechSynthesis.cancel(); }
        }
        function startSpeechRecognition() {
            window.SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
            if (!window.SpeechRecognition) { alert("Chrome use karein."); return; }
        ...
        async function sendMessage() {
            ...
            body.innerHTML += `<div class="msg-wrapper user-wrapper"><div class="bubble">${text}</div><div class="msg-meta">Tum • ${time}</div></div>`;
            body.scrollTop = body.scrollHeight;
            let loadingId = "load-" + Date.now();
            body.innerHTML += `<div class="msg-wrapper ai-wrapper" id="${loadingId}"><div class="bubble typing-dots"><span></span><span></span><span></span></div></div>`;
            body.scrollTop = body.scrollHeight;
            try {
                let res = await fetch("/get?msg=" + encodeURIComponent(text));
                let data = await res.text();
                document.getElementById(loadingId).remove();
                body.innerHTML += `<div class="msg-wrapper ai-wrapper"><div class="bubble">${data.replace(/\\n/g, '<br>')}</div><div class="msg-meta">MY-VIP-AGENT • ${time}</div></div>`;
                speakText(data);
            } catch(err) {
                document.getElementById(loadingId).remove();
                body.innerHTML += `<div class="msg-wrapper ai-wrapper"><div class="bubble" style="color:#ef4444;">Connection Error!</div></div>`;
            }
            body.scrollTop = body.scrollHeight;
        }
    </script>
